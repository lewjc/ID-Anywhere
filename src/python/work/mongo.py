...
class MongoWorkQueue():
    ...
    def __init__(self, host="127.0.0.1", port=27017, parent=""):
        try:
            ...
            print(
                "[{0} CONNECTED TO MONGO DB INSTANCE @ {1}:{2}]\n".format(
                    parent, host, port))
    ...
    def create_test_job(self):
        ...
        passport_image = self.__get_bytes(
            os.path.join(dir, "../passport.jpg"))
        passport_image = self.__gridfs.put(passport_image)
        me_image = self.__get_bytes(
            os.path.join(dir, "../me.jpg"))
        me_image = self.__gridfs.put(me_image)
        license_image = self.__get_bytes(
            os.path.join(dir, "../license.jpg"))
